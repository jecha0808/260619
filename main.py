...
# 4. 사용자 입력 UI
str.write("### 👇 나의 MBTI를 선택해보세요!")

# 드롭다운 선택 상자 (가독성을 높이기 위해 알파벳 정렬)
mbti_list = sorted(list(mbti_jobs.keys()))
selected_mbti = str.selectbox("나의 성격 유형은 무엇인가요?", mbti_list, index=0)
...
if selected_mbti:
    ...
    # 풍선 효과 애니메이션으로 학생들의 흥미 유발!
    str.balloons()
    
    # 결과 헤더
    str.markdown(f"### 🎉 당신은 **{selected_mbti} ({info['title']})** 이군요!")
    str.info(f"💡 **성격 특징:** {info['desc']}")
    
    str.write("#### 🔍 추천하는 대표 직업군")
    
    # 2열(Column) 구조로 배치하여 깔끔하게 시각화
    cols = str.columns(2)
    ...
    # 하단 격려 메시지
    str.success("🌟 MBTI는 진로 탐색을 위한 참고용일 뿐이에요! 여러분의 가능성은 무궁무진합니다.")
